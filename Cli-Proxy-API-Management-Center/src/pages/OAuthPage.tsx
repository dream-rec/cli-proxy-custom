    ...
    try {
      const res = await oauthApi.startAuth(
        provider,
        {
          ...
          method: provider === 'kiro' ? (states[provider]?.method || 'builder-id') : undefined
        }
      );
          ...
          return (
            <div key={provider.id}>
              ...
              >
                ...
                {provider.id === 'kiro' && (
                  <div className={styles.geminiProjectField}>
                    <label className="label">{t('auth_login.kiro_method_label')}</label>
                    <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                      {['builder-id', 'google', 'github'].map((m) => (
                        <Button
                          key={m}
                          variant={state.method === m || (!state.method && m === 'builder-id') ? 'primary' : 'secondary'}
                          size="sm"
                          onClick={() => updateProviderState(provider.id, { method: m })}
                        >
                          {t(`auth_login.kiro_method_${m}`)}
                        </Button>
                      ))}
                    </div>
                  </div>
